...
function renderCertificates(certificates) {
  ...
  certGrid.innerHTML = certificates.map((c, i) => `
    <div class="cert-card reveal" data-index="${i}">
      ...
      <h3>${c.title}</h3>
      <p>${c.issuer}</p>
      <div class="cert-cta">${c.available ? 'view certificate →' : 'view details →'}</div>
    </div>
  `).join('');
}
...
function openModal(cert) {
  ...
  if (cert.available && cert.file_url) {
    const isPdf = cert.filename.toLowerCase().endsWith('.pdf');
    modalBody.innerHTML = isPdf
      ? `<iframe class="modal-viewer" src="${cert.file_url}"></iframe>`
      : `<img class="modal-viewer" style="object-fit:contain" src="${cert.file_url}" alt="${cert.title}">`;
  } else {
    modalBody.innerHTML = `
      <div class="modal-fallback">
        <h3>${cert.title}</h3>
        <p>No file uploaded yet on the server for:</p>
        <code>static/certificates/${cert.filename}</code>
        <p>Drop the certificate PDF/image there with that exact filename to preview it here.</p>
        ${cert.link ? `<p style="margin-top:18px"><a href="${cert.link}" target="_blank" rel="noopener" style="color:var(--cyan)">Open original certificate link →</a></p>` : ''}
      </div>`;
  }
}
...
document.getElementById('certGrid').addEventListener('click', (e) => {
  ...
  if (!card) return;
  openModal(CERTIFICATES[+card.dataset.index]);
});
